...
pushurl = "https://api.pushshift.io/reddit/search/comment/?q=!build-price&sort=desc&size=20&fields=id,body&subreddit=pythonforengineers"
...
for comment in comments:
    ...
    if id not in comment_replied_to:
        ...
        m = re.search("!build-price (.*)", comment["body"], re.IGNORECASE)
        if (m):
            print("Bot replying to : ", comment["body"])

            url= "https://www.romexchange.com/api?exact=false&item=" + urllib.quote_plus(m.group(1).replace(",","|"))
